# Docker image to run QGIS Master
# 20210321 - CC BY-SA - Miguel Sevilla-Callejo

FROM debian:buster

LABEL Name=qgis_dev Version=1.0
...
RUN echo "deb https://qgis.org/debian-nightly buster main" > /etc/apt/sources.list.d/qgis-dev.list
...
# `docker build -t msevilla00/qgis:dev -f qgis_dev_buster.dockerfile .`

# To RUN QGIS from container:
# xhost + 
# & docker run --rm -it --name qgisdev \
#   -v /datafolder:/home/qgis/data -v /tmp/.X11-unix:/tmp/.X11-unix \
#   -e DISPLAY=unix$DISPLAY msevilla00/qgis:dev qgis